 ...
 * 
 * slightly modified by Lars George to fit the needs for the game powerpong
 * 
 * @author <a href="http://mina.apache.org">Apache MINA Project</a>
 * @org.apache.xbean.XBean
 */
public class PongProtocolCodecFilter extends IoFilterAdapter
{
	/** A logger for this class */
	private static final Logger			LOGGER			= LoggerFactory.getLogger(PongProtocolCodecFilter.class);

	private static final Class<?>[]		EMPTY_PARAMS	= new Class[0];
	private static final IoBuffer		EMPTY_BUFFER	= IoBuffer.wrap(new byte[0]);

	private final AttributeKey			ENCODER			= new AttributeKey(
																PongProtocolCodecFilter.class,
																"encoder");
	...
	private final AttributeKey			DECODER_OUT		= new AttributeKey(
																PongProtocolCodecFilter.class,
																"decoderOut");
	private final AttributeKey			ENCODER_OUT		= new AttributeKey(
																PongProtocolCodecFilter.class,
																"encoderOut");
	 ...
	 */
	public PongProtocolCodecFilter(ProtocolCodecFactory factory)
	{
		if (factory == null)
		{
	 ...
	 */
	public PongProtocolCodecFilter(final ProtocolEncoder encoder,
	 ...
	 *            The class responsible for decoding the message
	 */
	public PongProtocolCodecFilter(final Class<? extends ProtocolEncoder> encoderClass,
							...
							WriteRequest writeRequest) throws Exception
	{
		...
		if (encoder == null)
		{
			throw new ProtocolEncoderException(
					"The encoder is null for the session " + session);
		}

		if (encoderOut == null)
		{
			throw new ProtocolEncoderException(
					"The encoderOut is null for the session " + session);
		}
		...
		try
		{
			...
			while (!bufferQueue.isEmpty())
			{
						...
						|| ((IoBuffer) encodedMessage).hasRemaining())
				{
					...
					WriteRequest encodedWriteRequest = new EncodedWriteRequest(
		...
		public WriteFuture flush()
		{
			...
			while (!bufferQueue.isEmpty())
			{
						...
						|| ((IoBuffer) encodedMessage).hasRemaining())
				{
					...
					nextFilter.filterWrite(session, new EncodedWriteRequest(
							encodedMessage, future,
			...
			if (future == null)
			{
				future = DefaultWriteFuture.newNotWrittenFuture(session,
						new NothingWrittenException(writeRequest));
			}
